...
public class RateController extends BaseRequiredAuthorizationController {
    ...
    protected void doAuthGet(HttpServletRequest request, HttpServletResponse response, Account acc) throws ServletException, IOException {
        ...
        try {
            ...
            request.getRequestDispatcher("ratestar.jsp").forward(request, response);
        } catch (Exception e) {
        }
    }
...
//processRequest(request, response);
        String course_id_raw = request.getParameter("course_id");
        String learner_id_raw = request.getParameter("learner_id");
        String subject_id_raw = request.getParameter("subject_id");
        String category_id_raw = request.getParameter("category_id");
        ...
        try {
            int course_id = Integer.parseInt(course_id_raw);
            int learner_id = Integer.parseInt(learner_id_raw);
            int subject_id = Integer.parseInt(subject_id_raw);
            int category_id = Integer.parseInt(category_id_raw);
            float rate = Float.parseFloat(rate_raw);
            Learner_Course course = new Learner_Course();
            course.setCourse_id(course_id);
            course.setLearner_id(learner_id);
            course.setRate(rate);
            c.updateRateCourse(course);
            response.sendRedirect("rate?category_id=" + category_id + "&subject_id=" + subject_id + "&course_id=" + course_id);
        } catch (Exception e) {
